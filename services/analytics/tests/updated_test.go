...
type updatedMockedStorage struct {
	es.Interface
	Client *elastic.Client `json:"client"`
}

func (m updatedMockedStorage) IsTeamMember(_ string) (teamMember bool, err error) {
	return false, nil
}

func (m updatedMockedStorage) AddMessage(_ globals.Message, _ ...string) (err error) {
	return nil
}

func (m updatedMockedStorage) QueryRangeMessages(_ string, _ string) ([]globals.Message, error) {
...
var _ = Describe("In", func() {
	Describe("Test handler for updated messages", func() {
		It("Should store the message update", func() {
			client := updatedMockedStorage{
				Client: nil,
			}
			...
			response, err := a.HandleUpdatedMessage(message)
