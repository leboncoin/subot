...
type newMessageMockedSender struct {
	slack.Interface
...
func (m newMessageMockedSender) GetMessageType(_ slack.Event) globals.MessageType {
	return globals.NewMessage
}

func (m newMessageMockedSender) ReplyToMessage(_ string, _ string, _ []interface{}) error {
	return nil
}

func (m newMessageMockedSender) GetEvent(e slack.Event) globals.Event {
	return m.GetMessage(e)
}

func (m newMessageMockedSender) GetMessage(_ slack.Event) globals.Message {
...
func (m newMessageMockedSender) IsValidToken(_ slack.EventRequest) bool {
	return true
}

func (m newMessageMockedSender) IsWatchedChannel(_ slack.Event) bool {
	return true
}
...
var _ = Describe("In", func() {
	...
	BeforeEach(func() {
		...
		mockAnalyticsServer = httptest.NewServer(http.HandlerFunc(func(res http.ResponseWriter, req *http.Request) {
			if req.RequestURI != fmt.Sprintf("/v1/analytics/%s", globals.NewMessage) {
				res.WriteHeader(500)
			}
			res.WriteHeader(200)
	...
	Describe("Test handler for new messages", func() {
		It("Should call the correct analytics api endpoint", func() {
			s := newMessageMockedSender{
				Channel:  slack.Chan{},
				Token:    "",
				BotToken: "",
				BotID:    "",
			...
			Expect(analyticsRes.Responses[0].Action).To(Equal(globals.ReplyMessage))
		})
	})
})
