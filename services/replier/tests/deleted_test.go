...
type deletedMockedSender struct {
	slack.Interface
...
func (m deletedMockedSender) GetMessageType(_ slack.Event) globals.MessageType {
	return globals.DeletedMessage
}

func (m deletedMockedSender) GetEvent(e slack.Event) globals.Event {
	return m.GetMessage(e)
}

func (m deletedMockedSender) DeleteResponseToMessage(_ string) error {
	return nil
}

func (m deletedMockedSender) GetMessage(_ slack.Event) globals.Message {
...
func (m deletedMockedSender) IsValidToken(_ slack.EventRequest) bool {
	return true
}

func (m deletedMockedSender) IsWatchedChannel(_ slack.Event) bool {
	return true
}
...
var _ = Describe("In", func() {
	...
	BeforeEach(func() {
		...
		mockAnalyticsServer = httptest.NewServer(http.HandlerFunc(func(res http.ResponseWriter, req *http.Request) {
			if req.RequestURI != fmt.Sprintf("/v1/analytics/%s", globals.DeletedMessage) {
				res.WriteHeader(500)
			}
			res.WriteHeader(200)
	...
	Describe("Test handler for deleted messages", func() {
		It("Should call the correct analytics api endpoint", func() {
			...
			Expect(analyticsRes.Responses[0].Action).To(Equal(globals.DeleteMessage))
		})
	})
})
